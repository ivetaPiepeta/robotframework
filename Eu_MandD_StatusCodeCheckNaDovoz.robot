...
Run Test With Resolution
    [Documentation]  Tento test otvorí prehliadač, načíta stránku na dovoz a overí HTTP status kódy pre všetky dostupné url - pre desktop aj mobil.
    [Arguments]  ${width}  ${height}
    Disable Insecure Request Warnings
    Create Session  autobazar  ${URL_dovoz}  verify=False
    ${response}  GET On Session  autobazar  /
    Log  HTTP status kód je: ${response.status_code}
    Should Be Equal As Numbers  ${response.status_code}  200
    Open Browser  ${URL_dovoz}  chrome
