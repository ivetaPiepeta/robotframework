...
*** Test Cases ***
Open Browser And Check Statuses of hrefs - desktop and mobile
...
Run Test With Resolution
    ...
    Log To Console  Starting test case with resolution ${width}x${height}
    Disable Insecure Request Warnings
    Create Session  autobazar  ${URL_tests}  verify=False
    ${response}  GET On Session  autobazar  /
    Log  HTTP status kód je: ${response.status_code}
    Should Be Equal As Numbers  ${response.status_code}  200
    Open Browser  ${URL_tests}  chrome
    ...
    Log To Console  Test case completed with resolution ${width}x${height}
